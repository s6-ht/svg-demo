...
import type { PropertyMetadata, PropertyParseOptions, StyleValueRegistry } from './types';
...
import { convertPercentUnit, parseDimensionArrayFormat } from './parser/dimension';
import { BaseStyleProps, ParsedBaseStyleProps, Shape } from '../displayObjects/types';
import { isFunction, isNil } from 'lodash';
import { runtime } from '../manager/GlobalRuntimeManager';
import { AABB } from '../shapes/AABB';
import { addVec3 } from '../utils/math';
import { Tuple3Number } from '../utils/types';
import { DisplayObject, EMPTY_PARSED_PATH } from '../displayObjects';
...
export const BUILT_IN_PROPERTIES: PropertyMetadata[] = [
  ...
  },
  {
    ...
    d: (nodeName: string) => {
      if (nodeName === Shape.PATH || nodeName === Shape.POLYGON) {
        return '4';
      }
      return '10';
    }
...
const GEOMETRY_ATTRIBUTE_NAMES = BUILT_IN_PROPERTIES.filter((n) => !!n.l).map((n) => n.n);
...
export class DefaultStyleValueRegistry implements StyleValueRegistry {
  ...
  processProperties(
    ...
    if (!runtime.enableCSSParsing) {
      ...
      if (
        ...
        runtime.CSSPropertySyntaxFactory['<coordinate>'].postProcessor(object, attributeNames);
      }
      if (!isNil(attributes.zIndex)) {
        runtime.CSSPropertySyntaxFactory['<z-index>'].postProcessor(object, attributeNames);
      }
      // @ts-ignore
      if (attributes.path) {
        runtime.CSSPropertySyntaxFactory['<path>'].postProcessor(object, attributeNames);
      }
      // @ts-ignore
      if (attributes.points) {
        runtime.CSSPropertySyntaxFactory['<list-of-points>'].postProcessor(object, attributeNames);
      }
      if (!isNil(attributes.offsetDistance)) {
        runtime.CSSPropertySyntaxFactory['<offset-distance>'].postProcessor(object, attributeNames);
      }
      if (attributes.transform) {
        runtime.CSSPropertySyntaxFactory['<transform>'].postProcessor(object, attributeNames);
      }
    ...
    if (!skipParse) {
      attributeNames.forEach((name) => {
        object.computedStyle[name] = this.parseProperty(name as string, object.attributes[name], object);
      });
    }
    ...
    attributeNames.forEach((name) => {
      ...
      if (name in object.computedStyle) {
        object.parsedStyle[name] = this.computeProperty(name as string, object.computedStyle[name], object);
      }
    });
  ...
  postProcessProperty(name: string, object: DisplayObject, attributes: string[]) {
    const metadata = propertyMetadataCache[name];
  ...
  private updateGeometry(object: DisplayObject) {
    ...
    if (geometryUpdater) {
      ...
      const halfExtents: Tuple3Number = [Math.abs(width) / 2, Math.abs(height) / 2, depth / 2];
      ...
      const expansion = nodeName === Shape.POLYGON || nodeName === Shape.PATH ? Math.SQRT2 : 0.5;
      ...
      (filter as ParsedFilterStyleProperty[]).forEach(({ name, params }) => {
        if (name === 'blur') {
          ...
          geometry.renderBounds.update(
            ...
            addVec3(geometry.renderBounds.halfExtents, geometry.renderBounds.halfExtents, [blurRadius, blurRadius, 0])
            // vec3.add(
      ...
      let usedOriginXValue =
        (flipY ? -1 : 1) * (transformOrigin ? convertPercentUnit(transformOrigin[0], 0, object) : 0);
      let usedOriginYValue =
        (flipX ? -1 : 1) * (transformOrigin ? convertPercentUnit(transformOrigin[1], 1, object) : 0);
      usedOriginXValue =
        usedOriginXValue - (flipY ? -1 : 1) * ((anchor && anchor[0]) || 0) * geometry.contentBounds.halfExtents[0] * 2;
      usedOriginYValue =
        usedOriginYValue - (flipX ? -1 : 1) * ((anchor && anchor[1]) || 0) * geometry.contentBounds.halfExtents[1] * 2;
      object.setOrigin(usedOriginXValue, usedOriginYValue);

      // FIXME setOrigin may have already dirtified to root.
      runtime.sceneGraphManager.dirtifyToRoot(object);
    }
  }
