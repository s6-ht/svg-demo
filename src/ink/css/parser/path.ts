import type { DisplayObject, ParsedPathStyleProps } from '../../displayObjects';
import {
  AbsoluteArray,
  CurveArray,
  PathArray,
...
export function parsePath(path: string | PathArray, object: DisplayObject): ParsedPathStyleProps['path'] {
  const result = (isString(path) ? memoizedParsePath(path) : internalParsePath(path)) as ParsedPathStyleProps['path'];
