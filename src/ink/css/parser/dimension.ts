...
import { isNil, isNumber, isString, memoize } from 'lodash';
import { rad2deg, turn2deg } from '../../utils/math';
import { Shape } from '../../displayObjects/types';
import { IElement } from '../../dom/types';
...
export function parseDimension(unitRegExp: RegExp, string: string): CSSStyleValue | undefined {
  ...
  return matchedUnits.map((unit) =>
    getOrCreateUnitValue(
      Number(string.replace(new RegExp('U' + unit, 'g'), '').replace(new RegExp(taggedUnitRegExp, 'g'), '*0')),
      unit
    )
  )[0];
}
...
export function parseDimensionArrayFormat(string: string | number | (string | number)[], size: number): number[] {
  let parsed: number[];
...
export function convertPercentUnit(valueWithUnit: CSSUnitValue, vec3Index: number, target: DisplayObject): number {
