 ...
 */
export const parseTransformOrigin = memoize((value: string | number[]): [CSSUnitValue, CSSUnitValue] => {
  if (isString(value)) {
    ...
    // eg. center bottom
    return [
      parseLengthOrPercentage(convertKeyword2Percent(values[0])) as CSSUnitValue,
      parseLengthOrPercentage(convertKeyword2Percent(values[1])) as CSSUnitValue
    ];
  } else {
